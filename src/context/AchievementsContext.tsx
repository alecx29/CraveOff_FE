  ...
  const mergeWithDefinitions = (serverList: any[]): AchievementItem[] => {
    try {
      ...
      const merged = milestoneDefs.map(def => {
        ...
        const isStarterMilestone = def.threshold === 0;
        const unlocked: boolean = isStarterMilestone
          ? serverItem?.unlocked !== false // starter badge (STREAK_0) should appear unlocked unless explicitly disabled
          : typeof serverItem?.unlocked === 'boolean'
            ? !!serverItem.unlocked
            : false;
