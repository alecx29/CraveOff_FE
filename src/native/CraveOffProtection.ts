...
type NativeModuleShape = {
  ...
  configureWebsites?(maxCount: number): Promise<ConfigureWebsitesResult>;
  status(): Promise<Status>;
  openPrivateDnsSettings?(): Promise<boolean>;
  openSystemSettings?(): Promise<boolean>;
  authorizationStatus?(): Promise<
    "approved" | "denied" | "notDetermined" | "unknown" | "unavailable"
  >;
};
...
const emitter =
  Platform.OS === "android"
    ? new NativeEventEmitter(NativeModules.CraveOffProtection)
    : undefined;
...
export const CraveOffProtection = {
  ...
  authorizationStatus: async (): Promise<
    "approved" | "denied" | "notDetermined" | "unknown" | "unavailable"
  > => {
  ...
  configureWebsites: async (
    maxCount: number
  ): Promise<ConfigureWebsitesResult> => {
    if (!Native || !Native.configureWebsites) throw new Error(LINKING_ERROR);
    return Native.configureWebsites(maxCount);
  },
  ...
  testResolve: async (
    qname: string
  ): Promise<"BLOCKED" | "SAFESEARCH" | "FORWARDED"> => {
    if (!Native) throw new Error(LINKING_ERROR);
